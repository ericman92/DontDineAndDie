var SpiderController = function () {
    ...
    var BugDispatch = {
            ...
            initialize: function (a) {
                ...
                if ("transform" in document.documentElement.style) 
                    ...
                    if (this.transform) {
                    this.bugs = [];
                    for (var b = "multiply" === this.options.mouseOver
                        ? this.options.minBugs
                        : this.random(this.options.minBugs, this.options.maxBugs, !0), c = 0, d = this, c = 0; c < b; c++) {
                    ...
                    for (c = 0; c < b; c++) 
                        ...
                        e = this.bugs[c],
                        this.spawnDelay[c] = setTimeout(function (a) {
                            return function () {
                                d.options.canFly
                                    ? a.flyIn()
                                    : a.walkIn()
                ...
                if (a = a || window.event) {
                    ...
                    a.client && a.client.x
                        ...
                        : a.clientX
                            ...
                            : a.page && a.page.x
                                ? (b = a.page.x - (document.body.scrollLeft + document.documentElement.scrollLeft), c = a.page.y - (document.body.scrollTop + document.documentElement.scrollTop))
    ...
    var Bug = {
                ...
                if (this.animating && this.alive && this.active) {
                    ...
                    if (!(40 > c || (200 < c && (c = 200), this._lastTimestamp = a, 0 >= --this.toggle_stationary_counter && this.toggleStationary(), this.stationary))) {
                        if (0 >= --this.edge_test_counter && this.bug_near_window_edge() && (this.angle_deg %= 360, 0 > this.angle_deg && (this.angle_deg += 360), 15 < Math.abs(this.directions[this.near_edge] - this.angle_deg))) {
                            ...
                            this.large_turn_angle_deg = Math.abs(a) < Math.abs(d)
                                ? a
                                : d;
                            this.edge_test_counter = 10;
                        ...
                        if (0 >= --this.small_turn_counter) 
                            ...
                            if (0 < this.large_turn_angle_deg && 0 > a || 0 > this.large_turn_angle_deg && 0 < a) 
                                a = -a;
            ...
            drawBug: function (a, b) {
                ...
                this.bug && (a && b
                    ? this.setPos(a, b)
                    : this.setPos(this.bug.top, this.bug.left), this.inserted || (this.inserted = !0, document.body.appendChild(this.bug)))
            },
            ...
            walkFrame: function () {
                this.bug.style.backgroundPosition = -1 * this.walkIndex * this.options.bugWidth + "px " + (this.wingsOpen
                    ? "0"
                    : "-" + this.options.bugHeight + "px");
                this.walkIndex++;
            ...
            fly: function (a) {
                ...
                if (10 > Math.abs(d) + Math.abs(e)) 
                    ...
                    if (c > a.left && 0 < g || c > a.left && 0 > g) 
                        g *= -1,
                        Math.abs(d) < Math.abs(g) && (g /= 4);
                    if (b < a.top && 0 > f || b > a.top && 0 < f) 
                        f *= -1,
                        Math.abs(e) < Math.abs(f) && (f /= 4);
                ...
                if (this.bug) {
                    ...
                    0 === a
                        ...
                        : 1 === a
                            ...
                            : 2 === a
                                ? (a = c + .3 * this.options.bugHeight, b *= Math.random())
                                : (a = Math.random() * c, b = -1.3 * this.options.bugWidth);
                    this.bug.style.backgroundPosition = -3 * this.options.bugWidth + "px " + (this.wingsOpen
                        ? "0"
                        : "-" + this.options.bugHeight + "px");
                    this.bug.top = a;
                    this.bug.left = b;
            ...
            bug_near_window_edge: function () {
                ...
                this.bug.left < this.options.edge_resistance
                    ...
                    : this.bug.left > document.documentElement.clientWidth - this.options.edge_resistance && (this.near_edge |= this.NEAR_RIGHT_EDGE);
        ...
        SpawnBug = function () {
            var a = {},
                b;
            for (b in Bug) 
                Bug.hasOwnProperty(b) && (a[b] = Bug[b]);
            return a
        },
        ...
        cloneOf = function (a) {
            ...
            var b = a.constructor(),
                c;
            for (c in a) 
                a.hasOwnProperty(c) && (b[c] = cloneOf(a[c]));
            return b
        };
